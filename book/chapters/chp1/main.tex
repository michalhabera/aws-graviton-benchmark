...
\abstract{
We show performance results executing the FEniCS Project finite element software
on Amazon Web Services (AWS) c7g and c7gn instances with Graviton3 processors.
Graviton3 processors implement the ARMv8.4-A instruction set and provide
Scalable Vector Extensions (SVE) for Single Instruction Multiple Data (SIMD)
operations. Comparing clang 18 and GCC 13 series compilers for compiling a
high-order Laplace finite element kernel our results show that both compilers
emitted FMLA instructions on SVE registers for automatically vectorized loops.
The c7gn instances include a Elastic Fabric Adaptor (EFA) interconnect with a
bandwidth of $\SI{200}{\giga\byte\per\second}$ for low-latency and
high-bandwidth Message Passing Interface (MPI) based parallel communication.  We
tested multi-node weak scalability of a Poisson solver up to 512 MPI processes.
We find the overall performance and weak scalability of the c7gn-based cluster
is similar to a dedicated AMD EPYC Rome x86-64 cluster installed at the
...
(AVX), Neon and Scalable Vector Extensions (SVE).  The current strategy of FFCX
to ensure that its kernels take advantage of is amenable to the compiler applying
automatic vectorisation, a process that automatically converts a scalar program
into a vectorised program.
...
assembly code and comparing runtime performance. Full details are given in
\autoref{tab:compiler-kernels}.
...
High-order kernels, which are expected to benefit from SIMD instructions,
...
vectorization speed-up (\textgreater 4x) is more significant with the Aion nodes.
...
assembly reveal:
\begin{itemize}
    \item For the low-order case the compiler optimization level
