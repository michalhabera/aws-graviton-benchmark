...
One important aspect for achieving reasonable kernel performance at runtime is
ensuring the compiled kernels contain calls to single instruction multiple data
(SIMD) operations. Widely available SIMD standards include Streaming SIMD Extensions (SSE), Advanced Vector Extensions (AVX), Neon and
Scalable Vector Extensions (SVE). 

Ensuring that the compiled kernels contain SIMD instructions can be acheived in
a number of different ways. The most direct method is to use compiler
intrinsics in the generated C code to directly call specific SIMD operations. This
