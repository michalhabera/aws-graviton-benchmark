...
ARM-based cloud computing instances for finite element computations.
Specifically, we show performance results executing the FEniCS Project finite
element software on AWS c7g and c7gn instances with Graviton3 processors. These
processors support ARMv8.4-A instruction set with Scalable Vector Extensions
(SVE) for efficient data operations. Both clang 18 and GCC 13 compilers
...
Testing a distributed memory parallel DOLFINx Poisson solver with up to 512 MPI
processes, we found that the performance and scalability of the AWS instances
are comparable to a dedicated AMD EPYC Rome cluster at the University of
Luxembourg. These findings demonstrate that ARM-based cloud instances,
specifically AWS Graviton3, are competitive for parallel, distributed memory
